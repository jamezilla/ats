...
void atsread_timepnt(t_atsread *x, t_floatarg time)
{
	...
	if(frame < 0)
	{
		...
		if(x->outnz == 1 && (int)x->atshead.type != 1 && (int)x->atshead.type != 2)	//we can and should output noise
		{
			for(i = 0; i < 25; i++)
       	 		{
	        		if(x->outnzbands[i] == 1){
...
void atsread_any(t_atsread *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	if(strcmp(s->s_name, "open") == 0)	//open data file
	{
		if(argc < 1)
		{
	...
	if(strcmp(s->s_name, "set") == 0 || strcmp(s->s_name, "add") == 0)	//set or add partials to output
	{
	...
	if(strcmp(s->s_name, "remove") == 0)	//remove partials
	{
		for(i = 0; i < argc; i++)
		{
			temp = atom_getsymbol(&argv[i]);
	...
	if(strcmp(s->s_name, "setnz") == 0 || strcmp(s->s_name, "addnz") == 0)	//set or add bands to output
	{
	...
	if(strcmp(s->s_name, "removenz") == 0)	//remove bands
	{
		for(i = 0; i < argc; i++)
		{
			temp = atom_getsymbol(&argv[i]);

 ...
 *str=0;
  ...
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->duration);
  gtk_entry_set_text(GTK_ENTRY(entrys[1]), str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->lowest_freq);
  gtk_entry_set_text(GTK_ENTRY(entrys[2]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->highest_freq); 
  gtk_entry_set_text(GTK_ENTRY(entrys[3]),str);    
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->freq_dev);
  gtk_entry_set_text(GTK_ENTRY(entrys[4]),str);
  *str=0;
  sprintf(str, " %9.4f ",(float)atsh_anargs->win_cycles);    
  gtk_entry_set_text(GTK_ENTRY(entrys[5]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->hop_size);  
  gtk_entry_set_text(GTK_ENTRY(entrys[6]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->lowest_mag);  
  gtk_entry_set_text(GTK_ENTRY(entrys[7]),str);
  *str=0;
  sprintf(str, " %d ",atsh_anargs->track_len); 
  gtk_entry_set_text(GTK_ENTRY(entrys[8]),str);
  *str=0;
  sprintf(str, " %d ",atsh_anargs->min_seg_len);  
  gtk_entry_set_text(GTK_ENTRY(entrys[9]),str);
  *str=0;
  sprintf(str, " %d ",atsh_anargs->min_gap_len);  
  gtk_entry_set_text(GTK_ENTRY(entrys[10]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->SMR_thres);
  gtk_entry_set_text(GTK_ENTRY(entrys[11]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->min_seg_SMR);
  gtk_entry_set_text(GTK_ENTRY(entrys[12]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->last_peak_cont);
  gtk_entry_set_text(GTK_ENTRY(entrys[13]),str);
  *str=0;
  sprintf(str, " %9.4f ",atsh_anargs->SMR_cont);
  gtk_entry_set_text(GTK_ENTRY(entrys[14]),str);    
  
  gtk_option_menu_set_history (GTK_OPTION_MENU (optionmenu1), atsh_anargs->win_type); 
  gtk_option_menu_set_history (GTK_OPTION_MENU (optionmenu2), atsh_anargs->type-1  ); 
...
{
  filesel(NULL, "insel");
  atsh_anargs->start          =0.0;
  gtk_entry_set_text(GTK_ENTRY(data[0]), " 0.0000");      
  atsh_anargs->duration       =0.0;
...
void set_aparam(GtkWidget *widget, gpointer data)
{
  ...
  *str=0;        
  ...
  case 1:
    atsh_anargs->start=atof(str);
    break;
  case 2:  
    atsh_anargs->duration=atof(str);
    break;
  case 3:  
    atsh_anargs->lowest_freq= atof(str);
    break;
  case 4:
    atsh_anargs->highest_freq=atof(str);
    break;
  case 5:  
    atsh_anargs->freq_dev=atof(str);
    break;
  case 6:
    atsh_anargs->win_cycles=atof(str);
    break;
  case 7:
    atsh_anargs->hop_size=atof(str);
    break;
  case 8:
    atsh_anargs->lowest_mag=atof(str);
    break;
  case 10:
    atsh_anargs->track_len=atoi(str);
    break;
  case 11:
    atsh_anargs->min_seg_len=atoi(str);
    break;
  case 12:
    atsh_anargs->min_gap_len=atoi(str);
    break;  
  case 13:
    atsh_anargs->SMR_thres=atof(str);
    break;
  case 14:
    atsh_anargs->min_seg_SMR=atof(str);
    break;
  case 15:
    atsh_anargs->last_peak_cont=atof(str);
    break;
  case 16:
    atsh_anargs->SMR_cont=atof(str);
    break;
  }
...
void do_analysis (GtkWidget *widget, gpointer data)
{
  ...
  *previous=0;
  ...
  /* open output file */
  outfile = fopen(out_ats_tittle, "wb");
  if (outfile == NULL) {
  ...
  if(sound != NULL) {
    ...
    ats_save(sound, outfile, atsh_anargs->SMR_thres, atsh_anargs->type);
    fprintf(stderr, "done!\n");
    /* close output file */
    fclose(outfile);
...
/////////LABELS////////////////////////////////////
  ...
  isfile_label=create_entry (1,4,12,13,window1,table1,"18",0.0,in_tittle,0,405);
  afile_label=create_entry (1,4,13,14,window1,table1,"19",0.0,out_ats_tittle,0,405);
  rfile_label=create_entry (1,4,14,15,window1,table1,"20",0.0,res_tittle,0,405);

  //statlab=create_label("READY",0,1,10,11,window1,table1,"18");
/////////ENTRYS////////////////////////////////////

  entrys[0] =create_entry (1,2,0,1,window1,table1,"1",atsh_anargs->start,NULL,0,90);
  entrys[1] =create_entry (1,2,1,2,window1,table1,"2",atsh_anargs->duration,NULL,0,90);
  entrys[2] =create_entry (1,2,2,3,window1,table1,"3",atsh_anargs->lowest_freq,NULL,0,90);
  entrys[3] =create_entry (1,2,3,4,window1,table1,"4",atsh_anargs->highest_freq,NULL,0,90);
  entrys[4] =create_entry (1,2,4,5,window1,table1,"5",atsh_anargs->freq_dev,NULL,0,90);
  entrys[5] =create_entry(1,2,5,6,window1,table1, "6" ,atsh_anargs->win_cycles,NULL,0,90);
  entrys[6] =create_entry(1,2,6,7,window1,table1, "7" ,atsh_anargs->hop_size,NULL,0,90); 
  entrys[7] =create_entry (1,2,7,8,window1,table1,"8",atsh_anargs->lowest_mag,NULL,0,90);

  entrys[8] =create_entry(3,4,0,1,window1,table1,"10" ,atsh_anargs->track_len,NULL,1,90); /*int*/
  entrys[9] =create_entry(3,4,1,2,window1,table1,"11",atsh_anargs->min_seg_len,NULL,1,90); /*int*/
  entrys[10]=create_entry(3,4,2,3,window1,table1,"12",atsh_anargs->min_gap_len,NULL,1,90); /*int*/
  entrys[11]=create_entry(3,4,3,4,window1,table1,"13",atsh_anargs->SMR_thres,NULL,0,90); 
  entrys[12]=create_entry(3,4,4,5,window1,table1,"14",atsh_anargs->min_seg_SMR,NULL,0,90);
  entrys[13]=create_entry(3,4,5,6,window1,table1,"15",atsh_anargs->last_peak_cont,NULL,0,90);
  entrys[14]=create_entry(3,4,6,7,window1,table1,"16",atsh_anargs->SMR_cont,NULL,0,90);
...
////////WINDOW TYPE OPTION MENU///////////////////////////////////////
  ...
  gtk_option_menu_set_history (GTK_OPTION_MENU (optionmenu1), (int)atsh_anargs->win_type); 
